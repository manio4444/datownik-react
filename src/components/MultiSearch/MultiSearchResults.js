...
const ResultsResult = ({
  ...
  <Table>
    <Table.Header className={'result__title'}>
      <Table.Row>
        <Table.HeaderCell>
          ...
          {urlTo && title && (
            <>
              typu <Link to={urlTo}>{title}</Link>
            </>
          )}
        </Table.HeaderCell>
        <Table.HeaderCell textAlign={'right'}>
          <Link to={urlTo} state={{ queryString: queryString }}>
            więcej >>
          </Link>
        </Table.HeaderCell>
      </Table.Row>
    ...
    {count > 0 ? (
      <ResultsContent
        className="data-cell-scroll"
        children={resultsComponent}
      />
    ) : (
      <ResultsContent className="no-data">
        No data found &nbsp;&nbsp;&nbsp;&nbsp; ¯\_( ͡° ͜ʖ ͡°)_/¯
      </ResultsContent>
    )}
  </Table>
);
