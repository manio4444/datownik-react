...
class ListCalendar extends Component {
    ...
    render() {
        ...
        for (let i = 0; i < state.placeholders; i++) {
            placeholdersRender.push(<ListSingleEvent key={i} placeholder {...this.props}/>)
        }

        return (
            <div className="events__list">

                {state.fetchingData && <React.Fragment>{placeholdersRender}</React.Fragment>}

                {!state.fetchingData && state.list.map((event) => {
                    return (
                        <ListSingleEvent
                            key={event.id}
                            {...event}
